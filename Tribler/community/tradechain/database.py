...
class TradeChainDB(Database):
    ...
    def _get(self, query, params):
        ...
        return TradeChainBlock(db_result) if db_result else None

    def _getall(self, query, params):
        db_result = self.execute(_header + query, params).fetchall()
        return [TradeChainBlock(db_item) for db_item in db_result]
    ...
    def crawl(self, public_key, sequence_number, limit=25):
        ...
        return self._getall(u"WHERE insert_time >= (SELECT MAX(insert_time) FROM trade_chain WHERE public_key = ? AND "
    ...
    def get_num_unique_interactors(self, public_key):
        ...
        db_query = u"SELECT SUM(CASE WHEN up > 0 THEN 1 ELSE 0 END) AS pk_helped, SUM(CASE WHEN down > 0 THEN 1 ELSE " \
                   ...
                   u"trade_chain WHERE public_key = ? GROUP BY link_public_key) helpers"
