"""
File contains all conversions for the TradeChain Community.
"""
from struct import pack, unpack_from, calcsize
from Tribler.dispersy.conversion import BinaryConversion
from Tribler.dispersy.message import DropPacket

# Hash length used in the TradeChain Community
HASH_LENGTH = 32
SIG_LENGTH = 64
PK_LENGTH = 74
...
# total_btc total_mc Sequence_number, previous_hash
append_format = 'd d i ' + str(HASH_LENGTH) + 's'
# asset1 type, asset1 amount, asset2 type, asset2 amount
common_data_format = 'i d i d'
# [Up, Down,
...
class TradeChainConversion(BinaryConversion):
    """
    Class that handles all encoding and decoding of MultiChain messages.
    """

    def __init__(self, community):
        super(TradeChainConversion, self).__init__(community, "\x01")
        from Tribler.community.tradechain.community import SIGNATURE, CRAWL_REQUEST, CRAWL_RESPONSE, CRAWL_RESUME
    ...
    def _encode_signature(message):
        ...
        return pack(signature_format, *(payload.asset1_type, payload.asset1_amount,
...
def encode_block(payload, requester, responder):
    ...
    return pack(crawl_response_format, *(payload.asset1_type, payload.asset1_amount,
                                         payload.asset2_type, payload.asset2_amount,
                                         payload.total_btc_requester, payload.total_mc_requester,
                                         payload.sequence_number_requester, payload.previous_hash_requester,
                                         payload.total_btc_responder, payload.total_mc_responder,
...
def encode_block_requester_half(payload, public_key_requester, public_key_responder, signature_requester):
    return pack(requester_half_format, *(public_key_requester, public_key_responder,
                                         payload.asset1_type, payload.asset1_amount,
                                         payload.asset2_type, payload.asset2_amount,
                                         payload.total_btc_requester, payload.total_mc_requester,
                                         payload.sequence_number_requester, payload.previous_hash_requester,
                                         signature_requester))
